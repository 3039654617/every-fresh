<template>
  <div class="tabOne-wrapper" ref="oneTab">
      <div class="tab-item"
...
export default {
    ...
    methods: {
        ...
        scrollTo(i, e) {
            ...
            this.getSideList('酒水冲调');
        },
        mounted() {
          this.getSideList('酒水冲调');
        },
        moveTo(start, variable) {
            let dis = 0;
            let speed = 3;
            if(variable < 0){
                speed *= -1;
            ...
            const timer = setInterval(() => {
                ...
                if(Math.abs(dis) > Math.abs(variable)){
                    this.$refs.oneTab.scrollLeft = start + variable;
                    clearInterval(timer);
                };
            },4)
        }
...
.tabOne-wrapper::-webkit-scrollbar{
    width: 0;
    background: none;
}
</style>